...
block 10 'Provider credential enforcement' 'Proves the backend rejects absent or incorrect credentials and that a caller x-api-key cannot replace the projected provider credential.' 'The suite issued direct missing/wrong probes and a Gateway request with a conflicting caller x-api-key. Authorization override resistance is reported as not demonstrated because Authorization also authenticates the MaaS caller in this fixture.' "$(detail "$credential_re"); credential values=withheld" "$(status_for "$credential_re")" 'Direct HTTP 401 responses prove backend enforcement. An attributed HTTP 200 response with a conflicting caller x-api-key proves the projected Praxis provider credential remained authoritative for that header. Credential rotation, OAuth2, SigV4, and Authorization override resistance are not covered.'
