#!/usr/bin/env bash
# Patch synced MaaS deploy.sh for ai-gateway-controller e2e (no local maas-controller/ tree).
set -euo pipefail
...
if old not in text:
    # Already patched with an older marker block
    if marker in text:
        sys.exit(0)
...
# deploy-models.sh: scope AuthPolicy wait to MaaS-managed policies only (survives sync re-apply).
DEPLOY_MODELS_SH="${PROJECT_ROOT}/test/e2e/scripts/deploy-models.sh"
