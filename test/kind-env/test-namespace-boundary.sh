...
check_boundary() {
  ...
  local parent parent_name backend accepted resolved allowed secret_api grant_count
...
jq -n '{spec:{listeners:[{allowedRoutes:{namespaces:{from:"All"}}}]}}' >"$tmp/valid-gateway.json"
